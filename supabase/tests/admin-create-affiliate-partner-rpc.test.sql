...
-- ─────────────────────────────────────────────────────────────
-- TEST 4: Suspended partner re-invite → REFUSED (no silent un-suspend)
-- ─────────────────────────────────────────────────────────────
BEGIN;
DO $$
DECLARE
...
BEGIN
  ...
  -- Seed: partner exists and is suspended
  INSERT INTO public.partner_applications (
    user_id, legal_first_name, legal_last_name, phone_e164,
    bank_name, bank_account_no, bank_account_name, status
  ) VALUES (v_user, 'Suspended', 'Partner', '+66999999999', 'SCB', '0123456789', 'Suspended', 'approved')
  RETURNING id INTO v_app_id;
  INSERT INTO public.partners (user_id, application_id, commission_rate, approved_at, suspended_at, suspended_reason)
    VALUES (v_user, v_app_id, 0.3, now() - interval '7 days', now() - interval '3 days', 'fraud');

  BEGIN
    PERFORM public.admin_create_affiliate_partner_atomic(
      p_user_id => v_user, p_actor_id => v_actor,
      p_actor_email => 'admin@example.com', p_invited_email => 'suspended@example.com',
      p_legal_first_name => 'Re', p_legal_last_name => 'Invite', p_phone_e164 => '+66811112222',
      p_bank_name => 'SCB', p_bank_account_no => '0123456789', p_bank_account_name => 'Re',
      p_social_profile_url => '', p_social_platform => '', p_follower_count => 0,
      p_commission_rate => 0.3, p_code => 'MF-RE-INVITE', p_discount_percent => 20,
      p_campaign_label => 'Re'
    );
    RAISE EXCEPTION 'TEST 4 FAIL: re-invite of suspended partner was accepted';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM NOT LIKE 'partner_suspended%' THEN
      RAISE EXCEPTION 'TEST 4 FAIL: wrong exception "%"', SQLERRM;
    END IF;
  END;

  -- Partner row must still be suspended
  SELECT suspended_at INTO v_partner_status FROM public.partners WHERE user_id = v_user;
  IF v_partner_status IS NULL THEN
    RAISE EXCEPTION 'TEST 4 FAIL: suspended_at was silently cleared';
  END IF;

  RAISE NOTICE '✅ TEST 4 PASS: suspended partner re-invite refused, ban preserved';
END $$;
ROLLBACK;

-- ─────────────────────────────────────────────────────────────
-- TEST 5: Race-safe code conflict — code owned by partner A,
--         partner B tries to use it. ON CONFLICT DO UPDATE with
--         WHERE user_id=p_user_id must NOT touch partner A's row.
-- ─────────────────────────────────────────────────────────────
BEGIN;
DO $$
DECLARE
...
BEGIN
  ...
  -- Partner A owns MF-RACE with specific metadata we want to verify is UNTOUCHED
  INSERT INTO public.partner_applications (user_id, legal_first_name, legal_last_name, phone_e164, bank_name, bank_account_no, bank_account_name, status)
    VALUES (v_user_a, 'A', 'Owns', '+66811112222', 'SCB', '0123456789', 'A', 'approved') RETURNING id INTO v_app_a;
  INSERT INTO public.partners (user_id, application_id, commission_rate, approved_at) VALUES (v_user_a, v_app_a, 0.3, now());
  INSERT INTO public.referral_codes (user_id, code, code_type, is_active, campaign_label, discount_percent)
    VALUES (v_user_a, 'MF-RACE', 'partner_affiliate', true, 'A_owns', 15);

  BEGIN
    PERFORM public.admin_create_affiliate_partner_atomic(
      p_user_id => v_user_b, p_actor_id => v_actor,
      p_actor_email => 'admin@example.com', p_invited_email => 'b@example.com',
      p_legal_first_name => 'B', p_legal_last_name => 'New', p_phone_e164 => '+66811112222',
      p_bank_name => 'SCB', p_bank_account_no => '0123456789', p_bank_account_name => 'B',
      p_social_profile_url => '', p_social_platform => '', p_follower_count => 0,
      p_commission_rate => 0.3, p_code => 'MF-RACE', p_discount_percent => 99,
      p_campaign_label => 'B_tried_to_steal'
    );
    RAISE EXCEPTION 'TEST 5 FAIL: foreign-owned code was accepted';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM NOT LIKE '%already owned by another partner%' THEN
  ...
  -- Partner A's row metadata must be unchanged — proving ON CONFLICT DO
  -- UPDATE didn't fire on the foreign row even though the conflict matched.
  SELECT campaign_label, discount_percent INTO v_a_label, v_a_discount
    FROM public.referral_codes WHERE code = 'MF-RACE';

  IF v_a_label = 'A_owns' AND v_a_discount = 15 THEN
    RAISE NOTICE '✅ TEST 5 PASS: foreign owned code untouched by failed invite';
  ELSE
    RAISE EXCEPTION 'TEST 5 FAIL: partner A row mutated — label=%, discount=%', v_a_label, v_a_discount;
  END IF;
END $$;
ROLLBACK;
