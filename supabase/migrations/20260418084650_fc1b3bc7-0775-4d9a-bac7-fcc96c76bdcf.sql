-- Fix code_type mismatch: schema uses 'partner_affiliate', not 'partner_referral'
...
BEGIN
  ...
  -- Cycle cap (1-12 = full rate, 13+ = no commission)
  ...
  -- Only partner_affiliate earns commission (DB schema term)
  IF v_referral.code_type <> 'partner_affiliate' THEN
    RETURN NULL;
  END IF;

  -- Active partner = approved AND not suspended
