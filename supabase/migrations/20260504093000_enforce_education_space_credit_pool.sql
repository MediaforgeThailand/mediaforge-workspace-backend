-- Enforce class credit-pool limits at the database layer for every
-- education-space grant path. The UI already warns, but QR scans and teacher
-- manual grants must be protected even if a client bypasses the UI.
...
BEGIN
  ...
  v_pool_remaining := GREATEST(v_class.credit_pool - v_class.credit_pool_consumed, 0);
  IF v_credit_amount > v_pool_remaining THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error', 'class_budget_exhausted',
      'class_pool_remaining', v_pool_remaining
    );
  END IF;
...
BEGIN
  ...
  SELECT c.id, COALESCE(c.credit_pool, 0) AS credit_pool,
         COALESCE(c.credit_pool_consumed, 0) AS credit_pool_consumed
    INTO v_class
    FROM public.classes c
    WHERE c.id = v_space.class_id
      AND c.deleted_at IS NULL
    FOR UPDATE;
  ...
  IF v_delta > 0 THEN
    v_pool_remaining := GREATEST(v_class.credit_pool - v_class.credit_pool_consumed, 0);
    IF v_delta > v_pool_remaining THEN
      RETURN -1;
    END IF;
